...
$layout = tep_random_select("select id, title, language_id, logo, background_header, background_header_right, background_footer, status, cPath, page, date_scheduled, expires_date, background_site, background_site_content from " . TABLE_LAYOUT . " WHERE language_id IN('".$languages_id."','ALL') and page IN(0,'".tep_db_input($page_type)."') and id = '".$_GET['layout_id']."' ");
...
$layout = tep_random_select("select id, title, language_id, logo, background_header, background_header_right, background_footer, status, cPath, page, date_scheduled, expires_date, background_site, background_site_content from " . TABLE_LAYOUT . " WHERE language_id IN('".$languages_id."','ALL') and page = '0'and id = '".$_GET['layout_id']."' ");

}else{

$layout = tep_random_select("select id, title, language_id, logo, background_header, background_header_right, background_footer, status, cPath, page, date_scheduled, expires_date, background_site, background_site_content from " . TABLE_LAYOUT . " WHERE status = '1' and language_id IN('".$languages_id."','ALL') and page = '0' and template_folder = '".$layout_template_folder."' ");
	...
	$layout = tep_random_select("select id, title, language_id, logo, background_header, background_header_right, background_footer, status, cPath, page, date_scheduled, expires_date, background_site, background_site_content from " . TABLE_LAYOUT . " WHERE page = '0' and id = '".$_GET['layout_id']."' ");
...
/*		if (ENABLE_LIVEHELP == 'true') {
		...
		// Telephone Number
		if (PHONE_CALL != '') {
		echo '<td class="main" align="center" style="font-size:13px; line-height: 1.2; font-weight:bold;">' . tep_image(DIR_WS_IMAGES . 'telephone_32.png', '', '', ' align="absmiddle"') . ' ' . PHONE_CALL . '</td>';
		}
		// Telephone Number eof
	
		// MSN
		if (defined('VIEW_MSN') && VIEW_MSN == 'true') {
		echo '<td class="main" align="center" style="font-size:12px;"><a href="msnim:add?contact='.MSN.'">'.tep_image(DIR_WS_IMAGES . 'icon_msn.png', '', '', ' align="absmiddle"').'</a> <a href="msnim:add?contact='.MSN.'">' . MSN . '</a></td>';
		}
		// MSN eof
	
		// Skype
		if (VIEW_SKYPE == 'true') {
		echo '<td class="main" align="center" style="font-size:12px;"><a href="skype:'.SKYPE.'?add">'.tep_image(DIR_WS_IMAGES . 'icon_skype.png', '', '', ' align="absmiddle"').'</a> <a href="skype:'.SKYPE.'?add">' . SKYPE . '</a></td>';
		}
		// Skype eof
      ...
      <tr>
        ...
        <td width="10">&nbsp;</td>
        <td width="400" valign="top"><table border="0" cellpadding="0" cellspacing="5">
          <tr>
            <td>
	<?php
	// flag images payment
...
//            }
        ...
        ?>
              <td align="center"><table border="0" cellspacing="5" cellpadding="0" width="100%">
                ...
                ?>
                    <td align="center" class="main"><?php echo '<a href="'.LINK_MERCADO_LIVRE.'" target="_blank">' . tep_image(DIR_WS_IMAGES . 'logo_mercadolivre.png', 'Qualifica&ccedil;&otilde;es Mercado Livre', '72', '50', ' align="absmiddle"') . '</a>'; ?> </td>
                    <?php
				}
				?>
                    <?php
                ...
                ?>
                    <td align="center" class="main"><?php echo '<a href="'.LINK_TODA_OFERTA.'" target="_blank">' . tep_image(DIR_WS_IMAGES . 'logo_todaoferta.png', 'Qualifica&ccedil;&otilde;es Toda Oferta', '90', '50', ' align="absmiddle"') . '</a>'; ?> </td>
                    <?php
				}
				?>
                    <?php
                ...
                ?>
                    <td align="center" class="main"><?php echo '<a href="'.LINK_EBAY.'" target="_blank">' . tep_image(DIR_WS_IMAGES . 'logo_ebay.png', 'Qualifications eBay', '78', '50', ' align="absmiddle"') . '</a>'; ?> </td>
                    <?php
				}
				?>
                  </tr>
...
?>
	...
	echo '<a href="' . tep_href_link('sitemap.php') . '">' . tep_draw_separator('pixel_trans.gif', '1', '1') . '</a>'; ?>
          <?php echo '<a href="' . tep_href_link('sitemapcategories.php') . '">' . tep_draw_separator('pixel_trans.gif', '1', '1') . '</a>'; ?><?php echo '<a href="' . tep_href_link('sitemapproducts.php') . '">' . tep_draw_separator('pixel_trans.gif', '1', '1') . '</a>'; 
