...
if (PRODUCT_LIST_ONMOUSEOVER_BUTTONS == "true") {
	$add_button_buynow = '<div class="div_hidden" id="div_name'.$random_product['products_id'].'">'.$button_buynow.'</div>';
}else{
...
$add_ce = '</div>';
	  ...
	  $info_box_contents = $add_cs.'<span class="smallText"><div class="pl_style_border_div">'.$add_categories_subcategories.'<span class="pl_expire_date">'.$default_specials_expiredate.'</span>'.$products_image_list.$add_likes_box.'<br><br>'.$button_youtube.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $random_product['products_id']) . '">' . $manufacturers_name . $products_model . '<span class="pl_product_name">' . $random_product['products_name'] . '</span></a>'.$unidade_venda.'<br>'.$add_button_buynow.'</div></span>'.$add_ce;
