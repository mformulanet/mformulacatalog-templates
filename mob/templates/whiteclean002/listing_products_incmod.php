...
if ( ($random_product['products_quantity'] > 0) || ( ($random_product['products_quantity'] <= 0) && ($random_product['products_qtd_stock_status'] == '1') ) ) {
    ...
    if (tep_not_null($random_product['specials_new_products_price'])) {
	  ...
	  if (($random_product['parcel_window'] == 1) && ($language == 'portugues') && ($currency == 'BRL')) {
		...
		$whats_new_price = '<br>'.TEXT_PRICE_PRODUCT_FROM.'<s>' . $currencies->display_price($price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</s>';
		
		$whats_new_price .= '<br><span class="pl_product_special_price"><b>' . TEXT_PRICE_PRODUCT_TO . $currencies->display_price($newprice, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</b></span>';
			...
			$whats_new_price .= '<br>'.TEXT_PRICE_PRODUCT_OR.'<span class="pl_product_special_price"><b>' . $currencies->display_price($new_products_price_ok, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</b></span>' . ' ' . MODULE_PAYMENT_DISC_PERCENTAGE . '% ' . TEXT_OF_DISCOUNT . TEXT_PRICE_PRODUCT_AVISTA;
			...
			$whats_new_price .= '<br>'.TEXT_PRICE_PRODUCT_OR.'<span class="pl_product_special_price"><b>' . $currencies->display_price($new_products_price_ok, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</b></span>' . ' ' . MODULE_PAYMENT_DISC1_PERCENTAGE . '% ' . TEXT_OF_DISCOUNT . TEXT_PRICE_PRODUCT_AVISTA;
		...
		$whats_new_price = '<br><s>' . $currencies->display_price($price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</s><br>';
		$whats_new_price .= '<strong><span class="pl_product_special_price">' . $currencies->display_price($newprice, tep_get_tax_rate($random_product['products_tax_class_id'])) . '<br>'.NUMBER_PLOTS_VISA.'x ' . $price_divide . '</span></strong>';
	  ...
	  $whats_new_price = '<br><s>' . $currencies->display_price($price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</s><br>';
      $whats_new_price .= '<strong><span class="pl_product_special_price">' . $currencies->display_price($newprice, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</span></strong>';
	  ...
	  if (($random_product['parcel_window'] == 1) && ($language == 'portugues') && ($currency == 'BRL')) {
		...
		$prod_price = $random_product['products_price'];
		...
		$orig_price = $random_product['products_price'];
		...
		if (!tep_get_products_special_price($random_product['products_id'])) {
		$percentage_add = PERCENTAGE_FAKE_SPECIAL_PRICE_ALL_PRODUCTS;
		$price_to_add = round( ($percentage_add/100)*$random_product['products_price'], 0 );
		$new_price_add = $random_product['products_price'] + $price_to_add;
		$whats_new_price .= '<br>'.TEXT_PRICE_PRODUCT_FROM."<s>" . $currencies->display_price($new_price_add, tep_get_tax_rate($random_product['products_tax_class_id'])) . "</s>";
		}
		}
		// fake special price

		if ($discount > 0) {
		$whats_new_price .= '<br>'.TEXT_PRICE_PRODUCT_FROM.'<s>' . $currencies->display_price($orig_price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</s>';
		}
				
		$whats_new_price .= '<br><span class="pl_product_price"><b>' . TEXT_PRICE_PRODUCT_TO . $currencies->display_price($prod_price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</b></span>';
		...
		if ($discount > 0) {
		$whats_new_price .= '<br><s>'.$currencies->display_price($orig_price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</s>';
		}
		
		$whats_new_price .= '<br><strong><span class="pl_product_price">'.$currencies->display_price($prod_price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</span><br><span class="pl_product_plots_price"> '.NUMBER_PLOTS_VISA.'x ' . $price_divide . '</span></strong>';
		...
		$whats_new_price .= '<br><strong><span class="pl_product_price">'.$currencies->display_price($price, tep_get_tax_rate($random_product['products_tax_class_id'])) . '</span></strong>';
...
// products image
	  ...
	  if ($random_product['products_image_xl_1'] && ENABLE_SLIDESHOW_PRODUCTS_IMAGES == 'true' && file_exists(DIR_WS_IMAGES . $random_product['products_image_xl_1']) && CAROUSEL_DEFAULT_STYLE != 'style3') {
...
if (ENABLE_YOUTUBE_PRODUCTLISTING == 'true') {
	if (!empty($random_product['products_youtube'])) {
	  if (strstr($random_product['products_youtube'], "<iframe")) {
		...
		$button_youtube = '<a href="'.$link_watch_youtube.'" class="fancybox fancybox.iframe">'.tep_image(DIR_WS_IMAGES . 'icon_youtube_play.png', '', '', '', ' align="middle" ').'</a>';
		...
		$button_youtube = '<a href="'.str_replace("watch?v=", "embed/", $random_product['products_youtube']).'" class="fancybox fancybox.iframe">'.tep_image(DIR_WS_IMAGES . 'icon_youtube_play.png', '', '', '', ' align="middle" ').'</a>';
	  }
	}else{
		$button_youtube = '';
	}
...
$add_ce = '</div>';
	  ...
	  $info_box_contents = $add_cs.'<span class="smallText"><div class="pl_style_border_div">
										   ...
										   <br><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $random_product['products_id']) . '">' . $manufacturers_name . $products_model .'<span class="pl_product_name">' . $random_product['products_name'] . '</span></a>'.$unidade_venda.'
...
$add_ce = '</div>';
	  ...
	  $info_box_contents = $add_cs.'<span class="smallText"><div class="pl_style_border_div">'.$add_categories_subcategories.$products_image_list.$add_likes_box.'<br><br>'.$button_youtube.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $random_product['products_id']) . '">' . $manufacturers_name . $products_model . '<span class="pl_product_name">' . $random_product['products_name'] . '</span></a>'.$unidade_venda.'<span class="pl_expire_date">'.$specials_expiredate.'</div></span>'.$add_ce;
...
onmouseout="document.getElementById(\'div_name'.$random_product['products_id'].'\').style.display=\'none\';"><span class="pl_categories_name">' . $subcategories['categories_name'] . ' - ' . $categories['categories_name'] . '</span><br><br>'.$products_image_list.$add_likes_box.'<br><br>'.$button_youtube.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $random_product['products_id']) . '">' . $manufacturers_name . $products_model . '<span class="pl_product_name">' . $random_product['products_name'] . '</span></a>'.$unidade_venda.'<span class="pl_expire_date">'.$specials_expiredate.'</span><br>'.$button_buynow.'</div>');
