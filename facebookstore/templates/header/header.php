...
$layout = tep_random_select("select background_menu_header, title, language_id, logo, background_header, background_header_right, background_footer, status, cPath, page, date_scheduled, expires_date, background_site, background_site_content from " . TABLE_LAYOUT . " WHERE status = '1' ".$add_lng_id_qry." ".$add_temp_fld_qry."  and cPath IN(0,".tep_db_input($cPath_ok).") and page IN(0,1) and id = '".$_GET['layout_id']."' ");

}else{

$layout = tep_random_select("select background_menu_header, title, language_id, logo, background_header, background_header_right, background_footer, status, cPath, page, date_scheduled, expires_date, background_site, background_site_content from " . TABLE_LAYOUT . " WHERE status = '1' ".$add_lng_id_qry." ".$add_temp_fld_qry."  and cPath IN(0,".tep_db_input($cPath_ok).") and page IN(0,1) and template_folder = '".$layout_template_folder."' ");
...
$layout = tep_random_select("select background_menu_header, title, language_id, logo, background_header, background_header_right, background_footer, status, cPath, page, date_scheduled, expires_date, background_site, background_site_content from " . TABLE_LAYOUT . " WHERE status = '1' ".$add_lng_id_qry." ".$add_temp_fld_qry." and page IN(0,'".tep_db_input($page_type)."') and id = '".$_GET['layout_id']."' ");

}else{

$layout = tep_random_select("select background_menu_header, title, language_id, logo, background_header, background_header_right, background_footer, status, cPath, page, date_scheduled, expires_date, background_site, background_site_content from " . TABLE_LAYOUT . " WHERE status = '1' ".$add_lng_id_qry." ".$add_temp_fld_qry." and page IN(0,'".tep_db_input($page_type)."') and template_folder = '".$layout_template_folder."' ");
...
$layout = tep_random_select("select background_menu_header, title, language_id, logo, background_header, background_header_right, background_footer, status, cPath, page, date_scheduled, expires_date, background_site, background_site_content from " . TABLE_LAYOUT . " WHERE status = '1' ".$add_lng_id_qry." ".$add_temp_fld_qry." and page = '0' and id = '".$_GET['layout_id']."' ");

}else{

$layout = tep_random_select("select background_menu_header, title, language_id, logo, background_header, background_header_right, background_footer, status, cPath, page, date_scheduled, expires_date, background_site, background_site_content from " . TABLE_LAYOUT . " WHERE status = '1' ".$add_lng_id_qry." ".$add_temp_fld_qry." and page = '0' and template_folder = '".$layout_template_folder."' ");
	...
	if ($layout["background_menu_header"] != ""){ $background_menu_header = DIR_WS_IMAGES . $layout["background_menu_header"]; }
	...
	$layout = tep_random_select("select background_menu_header, title, language_id, logo, background_header, background_header_right, background_footer, status, cPath, page, date_scheduled, expires_date, background_site, background_site_content from " . TABLE_LAYOUT . " WHERE status = '1' and page = '0' ".$add_lng_id_qry." ".$add_temp_fld_qry." and id = '".$_GET['layout_id']."' ");
	
	}else{
	
	$layout = tep_random_select("select background_menu_header, title, language_id, logo, background_header, background_header_right, background_footer, status, cPath, page, date_scheduled, expires_date, background_site, background_site_content from " . TABLE_LAYOUT . " WHERE status = '1' and page = '0' ".$add_lng_id_qry." ".$add_temp_fld_qry." ");
...
// layout eof
?>
      <table width="100%" border="0" cellspacing="0" cellpadding="0" <?php if ($background_header != ""){ echo ' background="' . $background_header. '"'; }elseif(HEADER_BACKGROUND != ""){ echo ' bgcolor="'.HEADER_BACKGROUND.'"'; } ?>>
        <tr>
...
?>
<table width="100%" height="100" border="0" cellpadding="0" cellspacing="0" <?php if ($background_header != ""){ echo ' background="' . $background_header. '"'; }elseif(HEADER_BACKGROUND != ""){ echo ' bgcolor="'.HEADER_BACKGROUND.'"'; } ?>>
  <tr>
	...
	?>    </td>
	<td align="right" <?php if ($background_header_right != ""){ echo ' style="background-image:url('.$background_header_right.'); background-position:right; background-repeat:no-repeat;"'; } ?>>
      ...
      <tr>
        <td align="center"><table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td align="center"><table border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td><table width="250" border="0" cellpadding="0" cellspacing="5">
                        <?php
...
<script type="text/javascript" src="scripts/dropdowncontent.js"></script>
                    ...
                    <table width="100%" border="0" cellpadding="0" cellspacing="5">
                        <tr>
                          <td align="center" class="line_header_menu"><?php echo '<a href="'.tep_href_link(FILENAME_SHOPPING_CART).'">' . tep_image(DIR_WS_IMAGES . 'icon_carrinhocompras.png', '', '', '', ' align="middle" ') . '</a>'; ?> </td>
                          <td align="center"><?php 
				  if ($cart->count_contents() > 0) {
				  echo '<a href="'.tep_href_link(FILENAME_SHOPPING_CART).'" id="searchlink" rel="shopping_cart"><span class="line_header_menu"><font size="4">'.HEADER_TITLE_CART_CONTENTS . '</font> (' . $cart->count_contents() . ')' .'</span></a>'; 
				  }else{
				  echo '<a href="'.tep_href_link(FILENAME_SHOPPING_CART).'" id="searchlink" rel="shopping_cart"><span class="line_header_menu"><font size="4">'.HEADER_TITLE_CART_CONTENTS.'</font></span></a>'; 
				  }
				  ?>
...
?>
                  ...
                  <DIV id="shopping_cart" style="position:fixed; visibility: hidden; border: 1px solid <?php echo TABLE_BOX_BORDER_COLOR; ?>; background-color: #FFFFFF ; width: 300px; padding: 8px; z-index: 1050;">
...
?>
<table border="0" width="100%" height="40" cellspacing="0" cellpadding="0" bgcolor="#3b5998">
  <tr>
    <td align="center"><table width="100%" height="40" border="0" cellspacing="0" cellpadding="0">
      <tr>
...
<?php 
	include(DIR_WS_BOXES . 'manufacturers_banners1.php');
?>
<!-- manufacturers header eof -->
