...
// products image
	  ...
	  if ($specials['products_image_sm_1'] && ENABLE_SLIDESHOW_PRODUCTS_IMAGES == 'true' && file_exists(DIR_WS_IMAGES . $specials['products_image_sm_1'])) {
		$products_image_list = '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $specials['products_id']) . '" '.$add_style_slideshow_products_images.' itemprop="url">' . tep_image(DIR_WS_IMAGES . $specials['products_image'], $specials['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, $add_style_slideshow_products_images_img) . tep_image(DIR_WS_IMAGES . $specials['products_image_sm_1'], $specials['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';
	  }else{
		$products_image_list = '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $specials['products_id']) . '"  itemprop="url">' . tep_image(DIR_WS_IMAGES . $specials['products_image'], $specials['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';
	  }
// products image eof
...
// likes eof
	    ...
	    <tr>
		  ...
		  echo $button_youtube;
		  echo $products_model . '<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $specials['products_id']) . '" itemprop="url">' .$specials['products_name'] . '</a>'.$unidade_venda. '</td>';
		  echo '
...
// expire date and time
	  if ( ($specials['products_quantity'] > 0) && (DISPLAY_EXPIRE_DATE_SPECIALS == 'true') ) {
		  if ( (tep_not_null($specials['expires_date'])) && ($specials['expires_date'] != '0000-00-00 00:00:00')) {
			  ...
			  $specials_expiredate = TEXT_SPECIALS_EXPIREDATE . ' ' . tep_date_short($specials['expires_date']) . '<br>' . TEXT_SPECIALS_EXPIREDATE_FALTA . ' ' . $date_expire_replace . '<br>';
		  }else{
			  $specials_expiredate = '';
		  }
	  }
...
// products image
	  ...
	  if ($specials['products_image_sm_1'] && ENABLE_SLIDESHOW_PRODUCTS_IMAGES == 'true' && file_exists(DIR_WS_IMAGES . $specials['products_image_sm_1'])) {
		$products_image_list = '<center><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $specials['products_id']) . '" '.$add_style_slideshow_products_images.' itemprop="url">' . tep_image(DIR_WS_IMAGES . $specials['products_image'], $specials['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, $add_style_slideshow_products_images_img) . tep_image(DIR_WS_IMAGES . $specials['products_image_sm_1'], $specials['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a></center>';
	  }else{
		$products_image_list = '<center><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $specials['products_id']) . '"  itemprop="url">' . tep_image(DIR_WS_IMAGES . $specials['products_image'], $specials['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a></center>';
	  }
// products image eof
...
if (PRODUCT_LIST_ONMOUSEOVER_BUTTONS == "true") {
	$add_button_buynow = '<div id="div_name'.$specials['products_id'].'" style="display:none;"><br>'.$button_buynow.'</div>';
}else{
...
onmouseout="document.getElementById(\'div_name'.$specials['products_id'].'\').style.display=\'none\';">'.$add_categories_subcategories.'<span class="pl_expire_date">'.$specials_expiredate.'</span>'.$products_image_list.$add_likes_box.'<br>'.$button_youtube.'<a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $specials['products_id']) . '" itemprop="url">'.$manufacturers_name.$products_model . '<span class="pl_product_name">' . $specials['products_name'] . '</span></a>'.$unidade_venda.$add_button_buynow.'</div></td>' . "\n";
